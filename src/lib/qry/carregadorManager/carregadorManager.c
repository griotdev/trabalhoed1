...
CarregadorManager criaCarregadorManager() {
    ...
    if (!manager) { fprintf(stderr, "Erro alocar CarregadorManager\n"); exit(EXIT_FAILURE);}    
    manager->numCarregadores = 0;
    for (int i=0;i<MAX_CARREGADORES;i++){ manager->entradas[i].id=-1; manager->entradas[i].pilha=NULL; }
...
void destroiCarregadorManager(CarregadorManager m) {
    ...
    for (int i=0;i<manager->numCarregadores;i++){
        if (manager->entradas[i].pilha) destroiPilha(manager->entradas[i].pilha, NULL);
    }
    free(manager);
}
...
Pilha obtemCarregador(CarregadorManager m, int id){
    ...
    if (idx>=0) return manager->entradas[idx].pilha;
    if (manager->numCarregadores>=MAX_CARREGADORES){ fprintf(stderr,"Max carregadores excedido\n"); return NULL; }
    Pilha p = criaPilha();
